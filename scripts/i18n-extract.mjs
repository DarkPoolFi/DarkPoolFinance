...
const CLIENT = ["public/dashboard.js", "public/shielded.js", "public/app.js", "public/transparency.js", "public/venue-client.mjs", "src/shielded/client.ts", "src/shielded/ledger.ts", "src/shielded/prove.ts"];
const SERVER = walk("src/server/darkpool").filter((f) => f.endsWith(".ts") && !f.includes(".check."));
...
function literals(file, { onlyUserErrors }) {
  ...
  const visit = (n) => {
    ...
    if (text !== null) {
      ...
      const codeCall = inCall && CODE_CALLS.test(calleeName(inCall));
      const userError = inCall && ts.isNewExpression(inCall) && calleeName(inCall) === "UserError";
      ...
      if (!isKey && !codeCall && !inArray && (!onlyUserErrors || userError)) {
        ...
        const parts = /<\/?[a-z][^>]*>/i.test(text) ? [...text.matchAll(/(?:^|>)([^<>]*)(?=<|$)/g)].map((m) => m[1]) : [text];
