...
import { botReply, isTelegramAuthorized, siteLoader } from "@/server/darkpool/telegram";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const text = await botReply(m.text, lang, siteLoader(new URL(request.url).origin), m.chat.type === "private");
