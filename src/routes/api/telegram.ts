...
import { botReply, dbPings, isTelegramAuthorized, siteLoader } from "@/server/darkpool/telegram";
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        const text = await botReply(m.text, lang, siteLoader(new URL(request.url).origin), m.chat.type === "private", Date.now() / 1000, { id: m.chat.id, pings: dbPings });
